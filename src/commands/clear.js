  ...
  async execute(interaction) {
    ...
    try {
      ...
      const recent = messages.filter(
        (message) => Date.now() - message.createdTimestamp < TWO_WEEKS,
      );
      ...
      for (const message of old.values()) {
        try {
          ...
          logger.error(
            `Impossible de supprimer le message ${message.id} :`,
            error,
          );
        }
      }
      ...
      logger.info(
        `${deletedCount} message(s) supprimé(s) dans #${interaction.channel.name} par ${interaction.user.tag}.`,
      );
      await interaction.editReply(`🗑️ ${deletedCount} message(s) supprimé(s).`);
    } catch (error) {
